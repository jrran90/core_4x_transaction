...
const HelperFunctions = { createTermin, prefixMaker } = require('../../helpers/functions');
const FailedTransactionNotification = require('../failedTransaction/failed-transaction.model');
const { URL } = require('url');
const ipRangeCheck = require('ip-range-check');
const qs = require('querystring');
const NotificationQueue = require('../notificationQueue/notification-queue.model');
const AMQPConnection = require('../../helpers/amqp');
const messageBroker = new AMQPConnection();
 ...
 * @property {string} req.body.transactionNumber - The number of the invoice
 */
async function generateAuthentication(req, res, next){
	let paData = {};
	...
	if(req.query.shopify){
		...
		Object.keys(req.body.shopify)
	      ...
	      .forEach(function(v, i) {
	      	  if(v !== 'x_signature' && v !== 'x_gateway_reference' && v !== 'x_result' && v !== 'x_timestamp') signatureString += `${v}${decodeURIComponent(req.body.shopify[v])}`;
	       });

	    signatureString = signatureString.replace(/\+/g, ' ');

	    store = await Store.findOne({
			_id: req.body.shopify.x_account_id
		}).populate({path: 'merchant', model: Merchant, select: 'convenienceFee prefix name'});

		if(!store) return next(new APIError('You are not allowed here {account not found}', httpStatus.FORBIDDEN, true));

		const signatureHash = crypto.createHmac('sha256', store.salt).update(signatureString).digest('hex');
		if(signatureHash.toString() !== req.body.shopify.x_signature){
			return next(new APIError('You are not allowed here {invalid signature}', httpStatus.FORBIDDEN, true));
		}
		...
		const shopifySignature = crypto.createHmac('sha256', store.salt).update(shopifySignatureObject).digest('hex');
		...
		invoiceNumber = req.body.shopify.x_reference;
		paData.total = req.body.shopify.x_amount;
		total = req.body.shopify.x_amount;
	} else{
		invoiceNumber = req.body.transactionNumber;
		paData.total = req.body.total;
		total = req.body.total;

		// perform non-shopify authentication here
		if(!req.headers.authorization) return next(new APIError('You are not allowed here {account not found}', httpStatus.FORBIDDEN, true));

		const auth = new Buffer(req.headers.authorization.substring(6), 'base64').toString().split(':');
		const merchantId = auth[0];
		const merchantSecret = auth[1];

		if(merchantId == null || merchantId == '' || merchantSecret == null || merchantSecret == '') return next(new APIError('You are not allowed here', httpStatus.FORBIDDEN, true));
	...
	if(process.env.NODE_ENV === 'production') base_url = 'https://portal2.empatkali.co.id';
	else base_url = 'https://sandbox-portal.empatkali.co.id';

	if(store.portal) base_url = store.portal;

	if(req.query.customportal){
		if(req.query.customportal.indexOf('https') > -1) base_url = req.query.customportal;
	}


	if(store.merchant.prefix){
		transactionNumber = `${store.merchant.prefix}.${invoiceNumber.replace(/[^\w\s]/gi, '')}`
	} else{
		let prefix = prefixMaker(store.merchant.name);
		let accepted = false;
	    ...
	    while(!accepted) {
			if(await Merchant.count({prefix: prefix}) < 1){
				...
				await Merchant.findByIdAndUpdate(store.merchant._id, {$set: {prefix: prefix}});
				accepted = true;
			} else{
				prefix = prefixMaker(store.merchant.name, true);
			}
	    }
	...
	// looking for if the transaction already succeed
	const pat = await PreApprovedTransaction.findOne({transactionNumber: transactionNumber});

	if(pat){
		if(pat.status == 1 && !pat.pending) return next(new APIError('This transaction number is already created', httpStatus.FORBIDDEN, true));
	}

	// const inmd5 = crypto
	// 	    .createHash('md5')
	// 	    .update(`${parseInt(total)}${transactionNumber}${store._id}${store.salt}`)
	// 	    .digest('hex');
	const hashedKey = crypto.createHmac('sha256', store.salt).update(`${parseInt(total)}${transactionNumber}${store._id}${store.salt}`).digest('hex');
	// let qrData = `${store._id}|${total}|${inmd5}|${req.body.transactionNumber}|${mobileNumber}`;
	let qrData = `${store._id}|${total}|${hashedKey}|${transactionNumber}`;
	...
	if(keys.length > 4){
		...
		for(let i=0;i<keys.length;i++){
			if(counter < 5){
				if(keys[i] !== 'transactionNumber' && keys[i] !== 'total' && keys[i] !== 'secret' && keys[i] !== 'storeId' && keys[i] !== 'mobileNumber' && keys[i] !== 'failedURL' && keys[i] !== 'redirectURL' && keys[i] !== 'detail' && keys[i] !== 'terminDuration'){
					qrData += `|${keys[i]}:${req.body[keys[i]]}`;
					++counter;
				}
			}
	...
	if(req.body.redirectURL) paData.redirectURL = req.body.redirectURL;
	if(req.body.successURL) paData.successURL = req.body.successURL;

	paData.transactionNumber = transactionNumber;
	paData.store = store._id;

	// determining the termin duration
	if(store.defaultTerminDuration) paData.terminDuration = store.defaultTerminDuration;
	if(req.body.terminDuration) paData.terminDuration = req.body.terminDuration;

	if(req.body.detail) paData.detail = req.body.detail;

	const patDetail = await PreApprovedTransaction.update({transactionNumber: transactionNumber}, {$set: paData}, {upsert: true, new: true});
	const savedPAT = await PreApprovedTransaction.findOne({transactionNumber: transactionNumber});
	...
	if(baseURL.pathname !== '/'){
		redirect_url = new URL(baseURL.pathname, baseURL.href);
		redirect_url = new URL(`${redirect_url.pathname}/${savedPAT._id}/${hashedKey}`, redirect_url);
	} else{
		redirect_url = new URL(`/${savedPAT._id}/${hashedKey}`, baseURL.href);
	}
...
async function create(req, res, next){
	...
	let initCharge = await HelperFunctions.requester({
		url: config.midtrans.address + '/charge',
		method: 'POST',
		headers: {
	...
	await User.findOneAndUpdate({_id: user._id}, {$inc: {remainingCredit: -result.total }}, {upsert: true, setDefaultsOnInsert: true})
		.catch(async e => {
			...
			await HelperFunctions.requester({
				url: `${config.midtrans.address}/${order_id_creator}/cancel`,
				method: 'POST',
				headers: {
					Authorization: 'Basic ' + auth,
					'Content-Type': 'application/json'
		...
		merchantEmail: {
			...
			detail: {
				...
				properties: {
					name: merchant.name,
					transactionNumber: transactionNumber,
					createdAt : moment(result.termins[0].paid.date).format('MMM DD YYYY HH:mm'),
					amount: amount,
		...
		userEmail: {
			...
			detail: {
				...
				properties: {
					...
					total: amount,
					convenienceFee: cf.amount,
          contractUrl: `https://${(process.env.NODE_ENV === 'development') ? 'sb-' : ''}api.empatkali.co.id/user/api/approvedtransactions/trasactioncontract/${transactionNumber}`
				}
			}
		},
		userPushNotif: {
			...
			detail: {
				...
				body: `Kami telah menerima pembayaranmu untuk transaksi ${result.transactionNumber} sebesar Rp ${amount} (exc. biaya layaran Rp ${cf.amount})`
			}
		}
	}, { upsert: true });
...
async function danaCallback(req, res, next){
	...
	if(preApprovedTransaction.zilingo.is){
		const zilingoVoucher = await HelperFunctions.requester({
			...
			url: `http://mon.empatkali.co.id/zilingo/index.php`,
			method: 'POST',
			headers: {
				'Content-Type': 'application/json'
			},
		...
		// 	}
		// });
		const userEmailDetail = await NotificationQueue.create({
			type: 'userEmail',
			isTransaction: true,
			sent: false,
			user: user._id,
		...
		messageBroker.sendMessage('channelMailer', JSON.stringify(userEmailDetail));
	...
	// 	}
	// }, { upsert: true });
	const merchantEmailDetail = await NotificationQueue.create({
		type: 'merchantEmail',
		isTransaction: true,
		sent: false,
		user: user._id,
	...
	messageBroker.sendMessage('channelMailer', JSON.stringify(merchantEmailDetail));

	const userPushNotifDetail = await NotificationQueue.create({
		type: 'userPushNotif',
		isTransaction: true,
		sent: false,
		user: user._id,
	...
	messageBroker.sendMessage('channelFirebase', JSON.stringify(userPushNotifDetail));
	...
	// });
	const userPushNotif1Detail = await NotificationQueue.create({
		type: 'userPushNotif',
		isTransaction: true,
		sent: false,
		user: user._id,
	...
	messageBroker.sendMessage('channelFirebase', JSON.stringify(userPushNotif1Detail));



	// sending socket
	await HelperFunctions.broadcaster('successTransactionWithDana', user.mobileNumber, true, HelperFunctions.callbackMakerAdv(tl.transactionNumber, customFields, remainingCredit));
	// const username = 'empatkali';
	// const password = encodeURIComponent('vH<tV@@E7w.X3cZS');
	...
	// send callback to message broker
	merchantCallbackDetail = await NotificationQueue.create(merchantCallbackDetail);
	messageBroker.sendMessage('channelCallback', JSON.stringify(merchantCallbackDetail));
...
async function gopayCallback(req, res, next){
	...
	// sending socket
	await HelperFunctions.broadcaster('successTransactionWithGopay', user.mobileNumber, true, HelperFunctions.callbackMakerAdv(tl.transactionNumber, customFields, remainingCredit));
...
async function createFromPreApproved(req, res, next){
    ...
    // check if user already input cvv
    if(!preApprovedTransaction.cardToken && (preApprovedTransaction.paymentType !== 'dana' && preApprovedTransaction.paymentType !== 'dana')) return res.json(await sendCallback(body, merchant.callback, '206', '', tl));
	...
	if(preApprovedTransaction.zilingo.is){
		const zilingoVoucher = await HelperFunctions.requester({
			// uri: `http://mon.empatkali.co.id/terimalender${(process.env.NODE_ENV === 'production') ? '-prod' : ''}`,
			url: `http://${(process.env.NODE_ENV === 'development') ? 'sb-' : ''}mon.empatkali.co.id/zilingo/index.php`,
			method: 'POST',
			headers: {
				'Content-Type': 'application/json'
			},
		...
		// 	}
		// });
		const userEmail1Detail = await NotificationQueue.create({
			type: 'userEmail',
			isTransaction: true,
			sent: false,
			user: user._id,
		...
		messageBroker.sendMessage('channelMailer', JSON.stringify(userEmail1Detail));
	...
	if(dc.paymentGateway === 'xendit'){
		...
		initCharge = await HelperFunctions.requester({
			url: `${config.xendit.address}/credit_card_charges`,
			method: 'POST',
			headers: {
		...
		await HelperFunctions.requester({
			url: `http://mon.empatkali.co.id/terimalender${(process.env.NODE_ENV === 'production') ? '-prod' : ''}`,
			method: 'POST',
			headers: {
				'Content-Type': 'application/json'
			},
	...
	// send email of success transaction to user
	const userEmailDetail = await NotificationQueue.create({
		type: 'userEmail',
		isTransaction: true,
		sent: false,
	...
	// sending socket
	await HelperFunctions.broadcaster('successTransaction', user.mobileNumber, true, await sendCallback(body, '', '200', result._id, tl));
...
async function checkCallback(req, res, next){
	const preApprovedTransaction = await PreApprovedTransaction.findOne({transactionNumber: req.params.preApprovedTransactionId}).select('redirectURL');
	const approvedTransaction = await ApprovedTransaction.findOne({transactionNumber: req.params.preApprovedTransactionId}).select('_id');
	...
	const transactionNotification = await TransactionNotification.findOne({transactionNumber: req.params.preApprovedTransactionId});
 ...
 */
async function getTransactionProperties(req, res, next){
	const { transactionNumber = '' } = req.params;
	const preApprovedTransaction = await PreApprovedTransaction.findById(req.params.preApprovedTransactionId).select('failedURL redirectURL total successURL');
	...
	return res.json(preApprovedTransaction);
}
...
async function getFee(req, res, next){
	const preApprovedTransaction = await PreApprovedTransaction.findById(req.params.preApprovedTransactionId).select('store')
		.populate({
	  		path: 'store',
	  		model: Store,
	  		select: 'merchant'
	...
	if(!preApprovedTransaction) return next(new APIError('No transaction found', httpStatus.NOT_FOUND, true));

	const merchant = await Merchant.findById(preApprovedTransaction.store.merchant).select('convenienceFee');

	if(!merchant) return next(new APIError('No fee found', httpStatus.NOT_FOUND, true));

	return res.json({
		convenienceFee: merchant.convenienceFee
	});
}
...
async function sendCallback(body, url, status, tid, log, reRequest){
	...
	await TransactionLog.update({transactionNumber: body.transactionNumber}, log, {upsert: true});
