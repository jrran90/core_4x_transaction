...
const HelperFunctions = { callbackMakerAdv, broadcaster } = require('../../helpers/functions');
...
async function create(req, res, next){
	...
	// determining termin duration
	paData.terminDuration = 14;
	if(merchant.defaultTerminDuration) paData.terminDuration = merchant.defaultTerminDuration;
	if(req.body.terminDuration) paData.terminDuration = req.body.terminDuration || 14;
	...
	try{
		...
		if(req.query.store) broadcaster(userMobileNumber, true, {transactionNumber: transactionNumber, gross_amount: Math.ceil(amount / 4), store: true, paymentType: paData.paymentType});
		else broadcaster(userMobileNumber, true, {transactionNumber: transactionNumber, gross_amount: Math.ceil(amount / 4), paymentType: paData.paymentType});
...
async function updateCardToken(req, res, next){
	...
	if(user.defaultPayment === 'dana'){
		let cardTokenz = await HelperFunctions.requester({
			...
			qs: {
				...
				jml: Math.ceil(preApprovedTransaction.total / 4) + cf.amount,
	...
	// GOPAY
		...
		let cardTokenz = await HelperFunctions.requester({
 			url: `https://${(process.env.NODE_ENV === 'production') ? '' : 'sb-'}gopay.empatkali.co.id/charge.php`,
 			json: {
 				hp: user.mobileNumber,
 				amount: Math.ceil(preApprovedTransaction.total / 4) + cf.amount,
 		...
 		if(process.env.NODE_ENV === 'production'){
 			if(cardTokenz.status_code == '200') return res.json(cardTokenz);
 			else return next(new APIError(`Failed in creating gopay token: ${JSON.stringify(cardTokenz)}`, httpStatus.INTERNAL_SERVER_ERROR, true));
 		} else{
 			let chargeURL = '';
	 		if(cardTokenz.hasOwnProperty('actions')){
	 			...
	 			chargeURL = cardTokenz.actions[cardTokenz.actions.length - 1].url;
	 		}
	 		...
	 		return res.json({redirect_url: chargeURL});
 		}
		...
		if(dc.paymentGateway === 'xendit'){
			...
			let cardTokenz = await HelperFunctions.requester({
				...
				qs: {
					...
					gross_amount: Math.ceil(preApprovedTransaction.total / 4) + cf.amount,
...
async function createV2(req, res, next){
	...
	// 		});
	// } else{
	// 	if(!req.headers.authorization) return res.json(await sendCallbackLite(tl, '208'));
	...
	// 	if(merchantId == null || merchantId == '' || merchantSecret == null || merchantSecret == '') return res.json(await sendCallbackLite(tl, '208'));
	...
	// 	if(!checkStore) return res.json(await sendCallbackLite(tl, '208'));
	// }


	let store, userMobileNumber, amount, tokens, transactionNumber, signature, tn;
	...
	if(req.body.qr){
		...
		try{
			...
			const decodedToken = jwt.verify(token, config.userJwtSecret);
			if(decodedToken.dataToken){
				if(!decodedToken.dataToken.mobileNumber) return res.json(await sendCallbackLite(tl, '213'));
	...
	} else if(req.body.md5 || req.body.signature){
		if(!req.body.signature && !req.body.md5) return res.json(await sendCallbackLite(tl, '211'));

		const pat = req.preApprovedTransactions;

		if(!pat) return res.json(await sendCallbackLite(tl, '211'));

		transactionNumber = pat.transactionNumber; tn = pat.transactionNumber;
		store = pat.store;
		userMobileNumber = req.body.user_mobileNumber.trim();
		amount = pat.total || 0;
		tokens = req.body.token.trim();
	...
	// trap dino sell voucher/pulsa
	let trapper = config.skuTrap.split(',')
	let idxStore = 0
	for (idxStore in trapper) {
		let trap = trapper[idxStore].split(':')
		if(store.toString() === trap[0].toString()){
			let idx = 0
			let check = false
			const pat = req.preApprovedTransactions;
			for(idx in pat.detail.items) {
				check = new RegExp(trap[1], 'i').test(pat.detail.items[idx].sku_category);
				if(check){
					broadcaster(userMobileNumber, false, await sendCallbackLite(tl, '217'));
					return res.json(await sendCallbackLite(tl, '217'));
				}
			}
		}
	}
	...
	//     .update(`${amount}${tl.transactionNumber}${merchant._id}${merchant.salt}`)
	//     .digest('hex');
	const hashedKey = crypto.createHmac('sha256', merchant.salt).update(`${parseInt(amount)}${tl.transactionNumber}${merchant._id}${merchant.salt}`).digest('hex');

	// if in development, pass this validation
	if(req.body.qr){
		if(signature !== hashedKey){
			broadcaster(userMobileNumber, false, await sendCallbackLite(tl, '204'));
			return res.json(await sendCallbackLite(tl, '204'));
		}
	...
	// } 
		...
		const hk = req.body.md5 || req.body.signature;

		if(req.body.md5 !== hashedKey){
			broadcaster(userMobileNumber, false, await sendCallbackLite(tl, '204'));
			return res.json(await sendCallbackLite(tl, '204'));
		}
		if(!token){
...
/**
 * Creating transaction token
 */
async function updateCardTokenV2(req, res, next){
	const preApprovedTransaction = req.preApprovedTransactions;

	if(!preApprovedTransaction) return next(new APIError('No transaction found', httpStatus.INTERNAL_SERVER_ERROR, true));

	const store = await Store.findById(preApprovedTransaction.store)
					.populate({path: 'merchant', model: Merchant, select: 'convenienceFee prefix name'});
	...
	if(req.body.qr){
		...
		if(qrAssets.length < 4) return next(new APIError('Authentication failed: <md5>', httpStatus.FORBIDDEN, true));
		...
		md5 = req.body.md5 || req.body.signature;
	}

	// const inmd5 = crypto
	//     .createHash('md5')
	//     .update(`${preApprovedTransaction.total}${preApprovedTransaction.transactionNumber}${store._id}${store.salt}`)
	//     .digest('hex');
	const hashedKey = crypto.createHmac('sha256', store.salt).update(`${preApprovedTransaction.total}${preApprovedTransaction.transactionNumber}${store._id}${store.salt}`).digest('hex');

	if(req.body.md5 !== hashedKey) return next(new APIError('Authentication failed: <md5>', httpStatus.FORBIDDEN, true));

	if(!preApprovedTransaction.valid) return next(new APIError('This transaction is not valid yet', httpStatus.FORBIDDEN, true));
	...
	if(user.defaultPayment === 'dana'){
		let cardTokenz = await HelperFunctions.requester({
			...
			qs: {
				...
				hp: user.mobileNumber,
				// redirect: req.body.redirectUrl ? req.body.redirectUrl : 'https://empatkali.co.id'
				redirect: preApprovedTransaction.redirectURL ? preApprovedTransaction.redirectURL : 'https://empatkali.co.id'
			},
			method: 'GET',
			headers: {
				'Content-Type': 'application/json'
	...
	// CARD
		...
		if(cardToken.status_code === '200'){
			await PreApprovedTransaction.findOneAndUpdate({transactionNumber: preApprovedTransaction.transactionNumber}, {$set: {cardToken: cardToken.token_id}});
			return res.send(cardToken);
		} else{
			return next(new APIError('Failed in creating card token', httpStatus.INTERNAL_SERVER_ERROR, true));
		}
...
async function sendCallback(body, url, status, tid, log){
	...
	switch(status){
		...
		case '215':
			...
			break;
		case '217':
			callbackMessage.status_message = `can't do purchase for voucher`;
			break;
		default:
...
async function sendCallbackLite(body, status, pg){
	...
	// adding log
 	...
 	await new TransactionLog(body).save();
	return addingCustomField(body, callbackMessage);
}
